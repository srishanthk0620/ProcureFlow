...
export function BookingForm({group=false}:{group?:boolean}) {
 ...
 if(step===0){if(!centres.some(c=>c.id===centreId)){setError('wfInvalid');return}setStep(1);return}
 ...
 return <><WorkflowHeader title={group?'groupBooking':'bookSlot'}/>{group&&<p className="muted">Coordinate travel and processing together. Add a group name and a contact person.</p>}<ol className="wf-steps">{['centres','vDetails','wfReview'].map((key,i)=><li key={key} aria-current={step===i?'step':undefined}>{t(key as 'centres'|'vDetails'|'wfReview')}</li>)}</ol><form className="card wf-form" onSubmit={submit} noValidate>
 {step===0&&<><CommoditySelect value={commodity} onChange={value=>{setCommodity(value);setCentreId(recommendation(state,value)[0].id);setSlot('')}}/><label className="wf-field">{t('centres')}<select value={centreId} onChange={e=>{setCentreId(e.target.value);setSlot('')}}>{centres.map((c,i)=><option key={c.id} value={c.id}>{centreLabel(c.name)} · {c.total} {t('minutes')}{i===0?` · ${t('bestOption')}`:''}</option>)}</select></label><details className="wf-details"><summary>{t('wfWhy')}</summary><p>{t('vReason')}</p>{centres.map(c=><p key={c.id}>{centreLabel(c.name)}: {c.travel} + {c.wait} + {c.service} + {c.penalty} = {c.total} {t('minutes')}</p>)}</details><div className="pf-travel-summary"><strong>{t('vTravel')}</strong><span>~{centres.find(c=>c.id===centreId)?.travel} {t('minutes')} · {centres.find(c=>c.id===centreId)?.distance} {t('kilometres')}</span></div></>}
 {step===1&&<><label className="wf-field">{t('wfQuantity')}<input inputMode="numeric" value={quantity} maxLength={5} onChange={e=>setQuantity(maxDigits(e.target.value,5))} aria-describedby="qty-hint"/></label><p id="qty-hint" className="helper">50–20,000 kg</p>{group&&<><label className="wf-field">{t('wfGroupName')}<input maxLength={60} value={name} onChange={e=>setName(e.target.value)}/></label><label className="wf-field">{t('wfMembers')}<input inputMode="numeric" maxLength={2} value={members} onChange={e=>setMembers(maxDigits(e.target.value,2))}/></label><label className="wf-field">{t('wfContact')}<input maxLength={60} value={contact} onChange={e=>setContact(e.target.value)}/></label></>}<label className="wf-field">{t('wfDate')}<input type="date" value={date} min={dateOffset()} max={dateOffset(7)} onChange={e=>{setDate(e.target.value);setSlot('')}}/></label><fieldset><legend>{t('wfSlot')}</legend><div className="wf-slots">{demoSlots.map(time=>{const left=slotRemaining(state,centreId,date,time);return <label key={time} className={slot===time?'chosen':''}><input type="radio" name="slot" value={time} checked={slot===time} disabled={left===0||left<Number(quantity)} onChange={()=>setSlot(time)}/><strong>{time}</strong><span>{left===0?t('wfFullLabel'):left<Number(quantity)?t('wfLimited'):t('wfAvailable')} · {left} kg</span></label>})}</div></fieldset></>}
 {step===2&&<><h2>{t('wfReview')}</h2><dl className="wf-facts"><div><dt>{t('commodity')}</dt><dd>{t(commodity==='paddy'?'paddy':'wfCopra')}</dd></div><div><dt>{t('wfQuantity')}</dt><dd>{quantity} kg</dd></div><div><dt>{t('centres')}</dt><dd>{centreLabel(centres.find(c=>c.id===centreId)?.name || '')}</dd></div><div><dt>{t('appointment')}</dt><dd>{date} · {slot}</dd></div>{group&&<><div><dt>{t('wfGroupName')}</dt><dd>{name}</dd></div><div><dt>{t('wfMembers')}</dt><dd>{members}</dd></div><div><dt>{t('wfContact')}</dt><dd>{contact}</dd></div></>}</dl></>}
